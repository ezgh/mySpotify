...
const redirectUri = "https://myspotifyme.netlify.app";
let accessToken;
let userId;
...
const Spotify = {
  ...
  getAccessToken() {
    if (accessToken) {
      ...
      window.location.replace(
        `https://accounts.spotify.com/authorize?client_id=${clientId}&response_type=token&scope=${scopes}&redirect_uri=${redirectUri}`
      );
    }
  },
  ...
  checkCookies() {
    ...
    const tempCookie = document.cookie.match(
      "(^|;) ?" + "accessToken" + "=([^;]*)(;|$)"
    );
    const cookie = tempCookie ? tempCookie[2] : false;
    if (cookie) {
      // Checking time because Spotify only issues authorization for 60 minutes
      const accessedTime = document.cookie.match(
        "(^|;) ?" + "accessDate" + "=([^;]*)(;|$)"
      ); // This is for testing purposes
      let d = new Date().valueOf();
      if (d - accessedTime[2] > 3599999) {
        alert(
          "Your authorization with Spotify is expired. Please login again."
        );
        return false;
      }
    }
  ...
  savePlaylist(trackUris, time_range, recommendation) {
    ...
    if (recommendation) {
      ...
      if (time_range === "short_term") {
        name = "mySpotify recommendations for the last month";
      } else if (time_range === "medium_term") {
        name = "mySpotify recommendations for the last 6 months";
      } else if (time_range === "long_term") {
        name = "mySpotify recommendations for all time history";
      } else {
        return false;
      }
    } else {
  ...
  getRecommendations(artists, tracks) {
    ...
    return fetch(
      `https://api.spotify.com/v1/recommendations?seed_artists=${seedArtists}&seed_genres=${seedGenre}&seed_tracks=${seedTracks}&limit=50`,
      {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
